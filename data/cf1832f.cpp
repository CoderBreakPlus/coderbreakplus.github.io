	...
	for(int x=1;x<=n;x++){
		...
		for(int j=1;j<=t;j++){
			int val=calc(x,j);
			{
				...
				if(~pt){
					cc[0][j]+=val,cc[pt+1][j]-=val;
					int cut=lower_bound(pos+1,pos+pt+1,l[x]-m)-pos;
					cc[cut][j]-=m-l[x],cc[pt+1][j]+=m-l[x];
					cd[cut][j]--,      cd[pt+1][j]++;
				}
			}
			{
				int lft=R,rht=t,pt=t+1;
				while(lft<=rht){
					int mid=(lft+rht)>>1;
				...
				cc[pt][j]+=val;
				int cut=upper_bound(pos+pt,pos+t+1,r[x])-pos;
				cc[pt][j]-=r[x],cc[cut][j]+=r[x];
				cd[pt][j]++,    cd[cut][j]--;
			}
		}
	}
