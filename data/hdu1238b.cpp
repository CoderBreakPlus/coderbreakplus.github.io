// created time: 2026-08-20 13:25:59
#include<bits/stdc++.h>
using namespace std;
...
namespace stir{
	...
	void init(int k){
		for(int i=0;i<=k;i++){
			a[i]=(ull)inv[i]*qpow(i,k)%mod;
			b[i]=qpow(mod-1,i)*inv[i]%mod;
		}
		fastmul(a,b,s,k);
		for(int i=0;i<=k;i++)
			s[i]=(ull)s[i]*fac[i]%mod;
	}

	int calc(int k){
		int ans = 0;
		for(int j=1;j<=k;j++){
			ans = (ans + (ull)s[j] * qpow(mod+1-p,j-1) % mod
				* qpow(INV(p),j+1)) % mod;
		}
		return (ull)ans*p%mod;
	}
}
...
void procedure(){
	k=read(),p=read();
	stir::init(2*k);
	int ans1 = stir::calc(2*k);

	stir::init(k);
	int ans2 = stir::calc(k);

	printf("%d\n", (ans1+(ull)(mod-ans2)*ans2)%mod);
}
int main(){
	#ifdef LOCAL
