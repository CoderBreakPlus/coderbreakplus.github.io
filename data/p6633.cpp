...
poly tmp_poly(int x){
	poly ret = (x+1);
	for(int i=0;i<=x;i++)ret[i]=binom(x,i);
	return ret;
}
poly getval(int n){
	poly tmp=(vector<int>){1,1},ret;
	auto solve = [&](auto &&self,int l,int r)->poly{
		if(l==r) {
			int cf = binom(n-l*k+l,l);
			poly ret = (vector<int>){(l&1)?(mod-cf):(cf)};
			return ret;
		}
		int mid=(l+r)>>1;

		poly left = self(self,l,mid);
		poly right = self(self,mid+1,r);

		return left * tmp_poly((r-mid)*k) + (right << mid+1-l);
	};

	ret=solve(solve,0,n/k)*tmp_poly(n-n/k*k);
	return ret;
}

void solve(int w){
	poly tmp = getval(w+1)-getval(w);
	tmp.resize(w+1);
	poly a(w+1);
	for(int i=0;i<=w;i++) a[w-i]=tmp[i+1];
	all.pb(a);
}
...
void procedure(){
	...
	solve(w);
	conv(0,(int)all.size()-1);

	int ans=0;
	for(int i=0;i<m;i++){
		ans=(ans+(ull)all[0][i]*INV(binom(m-1,i)))%mod;
	}
