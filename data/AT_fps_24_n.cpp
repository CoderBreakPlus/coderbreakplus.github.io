// created time: 2026-08-11 18:33:29
#include<bits/stdc++.h>
using namespace std;
...
int n,a[250005],ans[250005],iv[250005];

void upd(int x,int v){
	for(int i=1;i*x<=n;i++){
		ans[i*x]=(ans[i*x]+(ull)iv[i]*v)%mod;
	}
}
void procedure(){
	n=read();
	for(int i=1;i<=n;i++){
		a[i]=min(n/i,(int)read());
		upd(i,1);
		upd(i*(a[i]+1),mod-1);
	}
	fastexp(ans,ans,n);
...
int main(){
	...
	ll T=1;
	math_init();
	for(int i=1;i<=250000;i++)iv[i]=(ull)inv[i]*fac[i-1]%mod;
	while(T--) procedure();
	return 0;
}