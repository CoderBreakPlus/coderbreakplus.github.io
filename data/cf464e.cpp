...
int check(int x,int y,int vx=0,int vy=0,int no=0){
	...
	vx^=t[x].tag, vy^=t[y].tag;
	// cout<<"checking "<<x<<" "<<y<<endl;
	if(t[x].sz == 1) return vx < vy;
	else{
		if(t[rc(x)].hsh[vx] == t[rc(y)].hsh[vy])
			return check(lc(x), lc(y), vx, vy, 1);
		else
			return check(rc(x), rc(y), vx, vy, 1);
	} 
}
// int checkf(int x,int y,int l=0,int r=M,int vx=0,int vy=0,int no=0){
// 	if(!no){
// 		if(t[x].hsh[0] == t[y].hsh[0])
// 			return -1;
// 	}
...
// 	if(t[x].sz == 1){
// 		cout<<"bit: "<<(t[x].tag^vx)<<" "<<(t[y].tag^vy)<<endl;
// 		return (t[x].tag^vx) < (t[y].tag^vy);
// 	}else{
// 		if(t[rc(x)].hsh[vx] == t[rc(y)].hsh[vy])
// 			return checkf(lc(x), lc(y), l, mid, vx^t[x].tag, vy^t[y].tag, 1);
// 		else
// 			return checkf(rc(x), rc(y), mid+1, r, vx^t[x].tag, vy^t[y].tag, 1);
// 	} 
// }
Node line;
...
void procedure(){
	...
	while(!q.empty()){
		...
		for(auto [y,w]: E[x]){
			...
			// 	print(0,M,rt[2]);puts("");
			// 	print(0,M,rt[4]);puts("");
			// 	int a=checkf(rt[2],rt[4]);
			// 	int b=checkf(rt[4],rt[2]);
			// 	cout<<a<<","<<b<<endl;
			// }
			q.emplace(y);
			// cout<<"now sta ";for(auto x:q)cout<<x<<" ";cout<<endl;
