// created time: 2026-05-19 13:48:24
#include<bits/stdc++.h>
using namespace std;
...
int n,m;
const int N = 2e5, B = 5e3;

int f[N/B+5][N+5], g[N+5];
int C[B+5][B+5];
void procedure(){
	n=read(),m=read();
	if(n<2*m){
	...
	n-=m;
	int res=(n-m)%B;
	int *b = f[(n-m)/B], *c = C[res];

	unsigned __int128 ans=0;
	for(int i=0;i<=res&&i<=n;i++)ans+=(ull)c[i]*b[n-i];
	int ret=(ull)(ans%mod)*binom(n,m)%mod*fac[m]%mod;
	printf("%d\n",ret);
}
int main(){
	#ifdef LOCAL
	...
	for(int i=0;i<=B;i++){
		for(int j=1;j<=i;j+=2)C[i][j]=mod-C[i][j];
	}
	for(int i=0;i<=B;i++){
		g[i]=binom(B,i);
		if(i&1)g[i]=mod-g[i];
