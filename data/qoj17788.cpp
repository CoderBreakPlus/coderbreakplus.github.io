...
struct Node{
	...
	void init(int N){
		...
		for(int i=0;i<n;i++){
			memset(b,0,sizeof(b));
			for(int j=2*i;j>=0;j--){
				upd(b[j+2],(ull)a[j]*c[2]);
				upd(b[j+1],(ull)a[j]*c[1]);
				upd(b[j],(ull)a[j]*c[0]);
			}
...
int yyz(int x){
	...
	return qpow(c[1],x)*qpow(c[0],n-4-x)%mod*binom(n-4,x)%mod;
}
void procedure(){
	n=read();
	if(n<=4){
	...
	for(int x=1;x<=4*n;x++){
		...
		// cout<<"x="<<x<<" ret="<<ret0<<"+"<<ret1<<"+"<<ret2<<"+"<<ret3<<"+"<<ret4<<endl;
