// created time: 2026-04-04 12:24:54
// E. Ball Dumping Golf
// https://qoj.ac/contest/3575/problem/17717
#include<bits/stdc++.h>
using namespace std;
...
int n,m,f[100005],g[100005],h[100005];

void procedure(){
	n=read(),m=read();
	for(int i=0;i<=n;i++){
		h[i]=fac[i*m]*qpow(inv[m],i)%mod;
		f[i]=1ull*h[i]*inv[i]%mod;
	}
	fastln(f,g,n);
	int ans=0;
	for(int i=1;i<=n;i++){
		ans=(ans+1ull*perm(n,i)*g[i]%mod*h[n-i])%mod;
	}
	// cout<<ans<<"/"<<h[n]<<endl;
	printf("%d\n",(int)(1ull*ans*INV(h[n])%mod));
}
int main(){
	#ifdef LOCAL
