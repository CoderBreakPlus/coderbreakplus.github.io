// created time: 2026-09-12 16:14:56
#include<bits/stdc++.h>
using namespace std;
...
int n,m,p[200005],q[200005],qq[200005],ans[200005];
int p1[200005],q1[200005];
char str[200005];
...
void procedure(){
	...
	for(int i=2;i<=m;i++){
		f[i]=f[i-1];
		while(f[i]&&str[1+f[i]]!=str[i]) f[i]=f[f[i]];
		if(str[1+f[i]]==str[i])f[i]++;
	}
